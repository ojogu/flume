...
export function JobsPage() {
  ...
  const { data, isLoading, isError, error, isFetching } = useQuery({
    ...
    queryFn: () => getJobs({
      ...
      api_key_id: activeApiKey || undefined,
      page,
      per_page: 20
    }),
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-[var(--bg-subtle)] hover:bg-[var(--bg-subtle)] border-none">
              ...
              {!activeApiKey && (
                <TableHead className="font-mono text-[10px] uppercase tracking-widest text-[var(--text-muted)]">API Key</TableHead>
              )}
            ...
            {isLoading ? (
              Array.from({ length: 5 }).map((_, i) => (
                <TableRow key={i}>
                  ...
                  {!activeApiKey && <TableCell><Skeleton className="h-4 w-20" /></TableCell>}
              ...
              <TableRow>
                <TableCell colSpan={activeApiKey ? 5 : 6} className="h-48 text-center">
              ...
              <TableRow>
                <TableCell colSpan={activeApiKey ? 5 : 6} className="h-48 text-center">
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <p className="text-xs text-[var(--text-muted)]">Jobs initiated via API will appear here.</p>
                  </div>
                </TableCell>
              </TableRow>
            ) : (
