...
export function AdminShell() {
  ...
  useEffect(() => {
    if (accessToken && user && !user.is_admin) {
      navigate('/admin/login', { replace: true })
    }
    ...
    <div className="flex min-h-screen bg-[var(--background)]">
      ...
      <aside className="hidden md:flex flex-col w-64 border-r border-[var(--border-subtle)] bg-[var(--bg-card)] sticky top-0 h-screen">
        ...
        <div className="flex-1 px-4">
          <p className="text-label text-[var(--text-muted)] mb-4 px-2 tracking-widest">Admin</p>
          <NavItems />
          <Link
            to="/dashboard"
            className="flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-subtle)] mt-2"
          >
