...
from src.service.api import WEB_SESSION_DAILY_LIMIT, ApiKeyService
...
async def create_session(
    ...
    """Create or refresh a short-lived session API key for anonymous web users (24h, 5 jobs/day).
    ...
    if raw_key and raw_key.startswith("web_"):
        ...
        if existing:
            count = await api_key_service.count_jobs_last_24h(existing.id)
            remaining = max(0, WEB_SESSION_DAILY_LIMIT - count)
            return success(
    ...
    return success(
        data=SessionResponse(
            ...
            jobs_remaining=WEB_SESSION_DAILY_LIMIT,
        ).model_dump(),
